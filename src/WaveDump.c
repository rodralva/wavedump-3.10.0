...
#define WaveDump_Release        "3.10.0 Ciemat 3.1"
#define WaveDump_Release_Date   "March 2021"
#define DBG_TIME
...
int WriteOutputFiles(WaveDumpConfig_t *WDcfg, WaveDumpRun_t *WDrun, CAEN_DGTZ_EventInfo_t *EventInfo, void *Event, uint32_t  ExtendedTT, int Nf, int NewFile)
{
    int ch, j, ns;
    CAEN_DGTZ_UINT16_EVENT_t  *Event16 = NULL;
    CAEN_DGTZ_UINT8_EVENT_t   *Event8 = NULL;
    ...
    for (ch = 0; ch < WDcfg->Nch; ch++) {
        ...
        if( WDcfg->OutFileFlags& OFF_BINARY) {
            ...
            BinHeader[2] = ch;
            BinHeader[3] = EventInfo->EventCounter;
            BinHeader[4] = (uint32_t)ExtendedTT;
            BinHeader[5] = EventInfo->TriggerTimeTag;
            //BinHeader[6] = EventInfo->Pattern;
            //BinHeader[7] = (uint64_t)Utime;
            if (!WDrun->fout[ch]) {
                char fname[100];
            ...
            // Ascii file format
            if (NewFile == 1 && WDrun->fout[ch]) {
                fclose(WDrun->fout[ch]);
                WDrun->fout[ch] = NULL;
            }
            if (!WDrun->fout[ch]) {
                char fname[100];
                sprintf(fname, "wave%d_%d.txt", ch, Nf);
            ...
            if( WDcfg->OutFileFlags & OFF_HEADER) {
                ...
                fprintf(WDrun->fout[ch], "Extended Trigger Time Stamp: %u\n", ExtendedTT); 
                fprintf(WDrun->fout[ch], "DC offset (DAC): 0x%04X\n", WDcfg->DCoffset[ch] & 0xFFFF);
                fprintf(WDrun->fout[ch], "ComputerTime: %2d \n", (uint32_t)Utime);
                fprintf(WDrun->fout[ch], "ComputerTime Format 2: %s", asctime(timeinfo));
...
{
    int AccumulatedEvents = 0, Nf = 0, NewFile = 0;
...
InterruptTimeout:
        ...
        if (WDrun.ContinuousWrite || WDrun.SingleWrite) {
            AccumulatedEvents += NumEvents;
            if (AccumulatedEvents > MaxEvents) {
                AccumulatedEvents = 0;
                Nf++;
                NewFile = 1;
            }
        ...
        for(i = 0; i < (int)NumEvents; i++) {
            ...
            char MyEventHeader[16];
            uint32_t  ExtendedTT;
            int  ExtendedTT_Int, ExInt0, ExInt1;
            int Stop = 1;
            //We copy the header sector that allocates the extender trigger time
            // It is needed to previously configure the ADC with WRITE_REGISTER 811C 00400000 00600000

            
            memcpy(MyEventHeader, EventPtr, sizeof(MyEventHeader));
            //ExtendedTT = (uint32_t)MyEventHeader[6] * 256 + MyEventHeader[5];
            ExInt0 = (int) MyEventHeader[5];
            ExInt1 = (int) MyEventHeader[6];
            if (ExInt0 < 0) {
                ExInt0 = ExInt0 + 256;
            }
            if (ExInt1 < 0) {
            ...
            ExtendedTT_Int = ExInt1* 256 + ExInt0;

            ExtendedTT = (uint32_t)ExtendedTT_Int;
            ...
            if (WDcfg.Nbit == 8) 
                ...
                if (WDrun.ContinuousWrite || WDrun.SingleWrite) {
                    ...
                    if (BoardInfo.FamilyCode == CAEN_DGTZ_XX742_FAMILY_CODE) {	
                        ...
                        if (NewFile == 1) NewFile = 0;
                    }
                    else if (WDcfg.Nbit == 8) {
                        ret = WriteOutputFiles(&WDcfg, &WDrun, &EventInfo, Event8, ExtendedTT, Nf, NewFile);
                        if (NewFile == 1) NewFile = 0;
                    }
                    else {
                        ret = WriteOutputFiles(&WDcfg, &WDrun, &EventInfo, Event16, ExtendedTT, Nf, NewFile);
                        if (NewFile == 1) NewFile = 0;
                    }
                    if (ret) {
